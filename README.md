...
New sessions inherit the caller's exported environment, including `PATH`, and start in the caller's current directory. The interactive shell can still change its environment through startup files. Reattaching preserves the running shell's environment rather than replacing it with the attaching terminal's environment. Aliases and unexported shell variables are not inherited.

Use `gtach list` or `gtach --list` without a TTY to show automatic sessions, their directory labels, socket paths, and status (`active`, `stale`, or `unreachable`). For explicitly named sessions, use `gtach list "$HOME/.gtach"` to inspect that socket directory. Listing probes connections without attaching or sending input, and never removes stale sockets. An active socket means a connection succeeded, not that a client is attached. Only sockets in the selected directory are listed; gtach does not scan the filesystem for custom sockets.

Each resolved directory path gets one session per user. Symlink aliases share a session; different directories get separate sessions. Changing directories inside the session does not change its identity, and renaming the original directory gives it a new identity. Automatic sockets live under `/tmp/gtach-<uid>/`, not in the project, in a directory owned by you with mode 0700. Directory paths are hashed to keep socket names short. Private `.json` sidecars store directory labels only, never environment variables; labels for finished sessions may remain but are not listed without a socket. Older sessions show `"-"` until bare `gtach` is run again from their directory; explicit sockets without metadata also show `"-"`. Existing unsafe directories and stale sockets are refused, not replaced.
...
| `--list`, `list` | List sessions; optionally pass a socket directory |
...
Set `Options.Replay` to retain and replay up to `gtach.ReplayLimit` (1 MiB) of recent output on each connection's first `Attach`. The library default is off, preserving live-only stream behavior. Replay and live output are ordered atomically; repeating `Attach` on one connection does not replay again.
...
CLI sessions retain the last 1 MiB of output in server memory, including output while detached; older bytes are discarded. Library sessions discard detached output unless `Options.Replay` is enabled. PTY output and environment variables are not logged to disk. Upgrading the client does not upgrade an already-running server: existing sessions keep their original replay limit (or stay live-only if created before replay support) until you finish their commands and start new sessions. `-c` and newly created `-A` sessions preserve initial output until the first client attaches. A library session with `WaitForClient` also waits if the command has already exited; cancel or close it if no client will attach. Slow clients are disconnected when their output queue (up to 256 KiB of live output plus a 1 MiB replay snapshot) fills or a write stalls for one second, rather than blocking the command or other clients. Final output drains for up to one second after command exit and queued socket output for up to one more second.
